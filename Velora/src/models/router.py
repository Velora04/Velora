"""
Enrutador neuronal para VELORA.

Este módulo implementa el sistema de enrutamiento que determina qué experto 
debe procesar cada entrada.
"""
import torch
...
class NeuralRouter(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.hidden_dim = config.router_hidden_dim
        ...
        self.math_task_classifier = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(config.dropout_rate),
            nn.Linear(self.hidden_dim // 2, 4)  # Tareas matemáticas: suma, resta, mult, div
        )
        ...
        self.language_task_classifier = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(config.dropout_rate),
            nn.Linear(self.hidden_dim // 2, 3)  # Tareas lenguaje: pregunta, comando, declaración
        )
