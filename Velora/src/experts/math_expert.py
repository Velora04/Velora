"""
Experto matemático para VELORA.

Este módulo implementa el experto especializado en operaciones matemáticas.
"""
import torch
...
class MathExpert(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.operation_detector = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(self.hidden_dim // 2, 4)  # 4 operaciones básicas
        )
        ...
        self.number_extractor = nn.Sequential(
            nn.Linear(self.hidden_dim, self.hidden_dim // 2),
            nn.ReLU(),
            nn.Linear(self.hidden_dim // 2, 2)  # Extrae 2 números
        )
